import React from 'react';
import { Link } from 'react-router-dom';

const Footer: React.FC = () => {
  return (
    <footer className="text-black py-5 mt-5" style={{backgroundColor: "#24cfa6"}}>
      <div className="container">
        <div className="row">
          {/* Column 1 - About Us */}
          <div className="col-md-3 mb-4">
            <h5 className="fw-bold text-dark">About Us</h5>
            <p>We are a leading e-commerce platform providing high-quality products at the best prices.</p>
          </div>

          {/* Column 2 - Quick Links */}
          <div className="col-md-3 mb-4">
            <h5 className="fw-bold text-dark">Quick Links</h5>
            <ul className="list-unstyled">
              <li>
                <Link to="/about" className="text-black">
                  <i className="bi bi-info-circle me-2"></i> About Us
                </Link>
              </li>
              <li>
                <Link to="/contact" className="text-black">
                  <i className="bi bi-envelope me-2"></i> Contact Us
                </Link>
              </li>
              <li>
                <Link to="/faq" className="text-black">
                  <i className="bi bi-question-circle me-2"></i> FAQ
                </Link>
              </li>
              <li>
                <Link to="/terms" className="text-black">
                  <i className="bi bi-file-earmark-text me-2"></i> Terms & Conditions
                </Link>
              </li>
            </ul>
          </div>

          {/* Column 3 - Customer Service */}
          <div className="col-md-3 mb-4">
            <h5 className="fw-bold text-dark">Customer Service</h5>
            <ul className="list-unstyled">
              <li>
                <Link to="/shipping" className="text-black">
                  <i className="bi bi-truck me-2"></i> Shipping Information
                </Link>
              </li>
              <li>
                <Link to="/returns" className="text-black">
                  <i className="bi bi-arrow-return-left me-2"></i> Returns & Exchanges
                </Link>
              </li>
              <li>
                <Link to="/support" className="text-black">
                  <i className="bi bi-headset me-2"></i> Customer Support
                </Link>
              </li>
            </ul>
          </div>

          {/* Column 4 - Follow Us */}
          <div className="col-md-3 mb-4">
            <h5 className="fw-bold text-dark">Follow Us</h5>
            <ul className="list-unstyled">
              <li>
                <a href="https://www.facebook.com" className="text-black" target="_blank" rel="noopener noreferrer">
                  <i className="bi bi-facebook me-2"></i> Facebook
                </a>
              </li>
              <li>
                <a href="https://www.instagram.com" className="text-black" target="_blank" rel="noopener noreferrer">
                  <i className="bi bi-instagram me-2"></i> Instagram
                </a>
              </li>
              <li>
                <a href="https://twitter.com" className="text-black" target="_blank" rel="noopener noreferrer">
                  <i className="bi bi-twitter me-2"></i> Twitter
                </a>
              </li>
            </ul>
          </div>
        </div>

        {/* Bottom Footer */}
        <div className="row mt-4">
          <div className="col text-center">
            <p className="text-dark">&copy; {new Date().getFullYear()} Your ECOM. All rights reserved.</p>
          </div>
        </div>
      </div>
    </footer>
  );
};

export default Footer;
